...
data = pd.read_csv(r"data/ex1data2.txt",names=['size','bedrooms','lable'])
...
X = x.values
Y = y.values
X = torch.tensor(X,dtype=torch.float32)       #将X,Y变成tensor
Y = torch.tensor(Y,dtype=torch.float32)

features,labels = X,Y.reshape(-1,1)     #找出特征值，及标签，将标签转化成列
